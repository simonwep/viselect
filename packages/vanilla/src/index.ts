...
export default class SelectionArea extends EventTarget<SelectionEvents> {
  ...
  constructor(opt: PartialSelectionOptions) {
    ...
    if (this._options.document instanceof ShadowRoot) {
      ...
      const startAreasArray = Array.isArray(this._options.startAreas)
        ? this._options.startAreas
        : [this._options.startAreas];
      ...
      const boundariesArray = Array.isArray(this._options.boundaries)
  ...
  _onSingleTap(evt: MouseEvent | TouchEvent): void {
    ...
    if (evt.shiftKey && range && this._latestElement) {
      ...
      (stored.length === 1 || evt.ctrlKey || stored.every((v) => this._selection.stored.includes(v)))
    ) {
    ...
    if (x2 < _targetRect.left + startScrollMargins.x) {
      ...
      _scrollSpeed.x =
        scrollWidth - scrollLeft - clientWidth
          ? abs(_targetRect.left + _targetRect.width - x2 - startScrollMargins.x)
          : 0;
    ...
    if (y2 < _targetRect.top + startScrollMargins.y) {
      ...
      _scrollSpeed.y =
        scrollHeight - scrollTop - clientHeight
          ? abs(_targetRect.top + _targetRect.height - y2 - startScrollMargins.y)
          : 0;
